...
export interface SpineFoldInput {
  ...
  readonly status?: Pick<
    SpineFoldStatus,
    'cursorId' | 'summary' | 'parentId' | 'parentSummary'
  >;
}
...
export function foldSpine(
  ...
  if (input.status !== undefined) {
    out.push(statusMessage(completeStatus(messages, out, input)));
  }
  return out;
}
...
function completeStatus(
  messages: readonly ContextMessage[],
  projected: readonly ContextMessage[],
  input: SpineFoldInput,
): SpineFoldStatus {
  const base = input.status!;
  const state = input.state;
  const cursor = state.nodes[base.cursorId];
  const cursorOpenedAt = cursor === undefined || cursor.openedAt < 0
    ? state.epochStartAt
    : cursor.openedAt;
  const projectedContext = estimateTokensForMessages(projected);
  return {
    ...base,
    cursorContext: estimateTokensForMessages(messages.slice(cursorOpenedAt)),
    contextLeft:
      input.maxContextTokens !== undefined && input.maxContextTokens > 0
        ? Math.max(0, input.maxContextTokens - projectedContext)
        : undefined,
    rawContext: estimateTokensForMessages(messages),
    projectedContext,
  };
}
...
function statusMessage(status: SpineFoldStatus): ContextMessage {
  ...
  const projectedContext = ` projected_context="~${formatTokens(status.projectedContext)}"`;
