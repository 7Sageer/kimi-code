...
* Spine transitions change task scope, not communication state. A final response,
  ...
  \`spine_next\`, or \`spine_close\` call; never create a reporting node or perform a
  transition solely for delivery.
...
export function appendSpineView(baseInstructions: string, view: string = SPINE_VIEW): string {
